...
object SettingsManager {
    ...
    fun getProxyMode(): ProxyMode {
        if (!isRootMode()) return ProxyMode.STANDARD
        return when (ProxyMode.fromValue(MmkvManager.decodeSettingsString(AppConfig.PREF_ROOT_PROXY_MODE))) {
            ProxyMode.TPROXY -> ProxyMode.TPROXY
            else -> ProxyMode.ROOT_TUN
        }
    }

    fun isTProxyMode(): Boolean = getProxyMode() == ProxyMode.TPROXY

    fun setProxyMode(mode: ProxyMode) {
        MmkvManager.encodeSettings(AppConfig.PREF_ROOT_PROXY_MODE, mode.value)
        MmkvManager.encodeSettings(AppConfig.PREF_ROOT_MODE_ENABLE, mode != ProxyMode.STANDARD)
    }
    ...
    private fun ensureDefaultSettings() {
        ...
        ensureDefaultValue(AppConfig.PREF_ROOT_PROXY_MODE, ProxyMode.STANDARD.value)
