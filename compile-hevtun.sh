...
# 3) Standalone transparent-proxy helper for rooted devices. The filename uses
# a lib*.so shape so Android extracts it into nativeLibraryDir with execute bits.
"$NDK_HOME/ndk-build" \
    NDK_PROJECT_PATH=. \
    APP_BUILD_SCRIPT=jni/hev-socks5-tproxy/Android.mk \
    "APP_ABI=$ABIS" \
    APP_PLATFORM=android-24 \
...
for abi in $ABIS; do
  cp "$TMPDIR/libs-tproxy/$abi/hev-socks5-tproxy-bin" \
    "$__dir/libs/$abi/libhevsockstproxy.so"
done
