...
class MainRepository(
    ...
    private val _trafficSnapshot = MutableStateFlow(TrafficStatsManager.snapshot.value)
    override val trafficSnapshot: Flow<TrafficSnapshot> = _trafficSnapshot

    private val serviceReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            val safeIntent = intent ?: return
            val key = safeIntent.getIntExtra("key", 0)
            if (key == AppConfig.MSG_TRAFFIC_UPDATE) {
                readTrafficSnapshot(safeIntent)?.let { _trafficSnapshot.value = it }
                return
            }
    ...
    private fun readTrafficSnapshot(intent: Intent): TrafficSnapshot? =
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            intent.getSerializableExtra("content", TrafficSnapshot::class.java)
        } else {
            @Suppress("DEPRECATION")
            (intent.getSerializableExtra("content") as? TrafficSnapshot)
        }
