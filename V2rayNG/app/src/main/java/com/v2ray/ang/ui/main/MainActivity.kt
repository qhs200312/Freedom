...
class MainActivity : HelperBaseComponentActivity() {
        ...
        MainScreen(
            ...
            onAction = { action ->
                when (action) {
                    ...
                    is MainAction.SetProxyMode -> handleProxyModeChange(action.mode)
    ...
    private fun handleProxyModeChange(mode: ProxyMode) {
        val state = mainViewModel.uiState.value
        if (state.isRunning || state.isCheckingRoot || state.proxyMode == mode) return

        if (mode == ProxyMode.STANDARD) {
            SettingsManager.setProxyMode(ProxyMode.STANDARD)
            mainViewModel.updateProxyMode(ProxyMode.STANDARD)
            return
        }

        mainViewModel.updateProxyMode(state.proxyMode, isCheckingRoot = true)
        lifecycleScope.launch {
            val rootAvailable = RootManager.refresh()
            val resolvedMode = if (rootAvailable) mode else ProxyMode.STANDARD
            SettingsManager.setProxyMode(resolvedMode)
            mainViewModel.updateProxyMode(
                proxyMode = resolvedMode,
                isCheckingRoot = false,
            )
            if (!rootAvailable) toastError(R.string.toast_root_required)
        }
    }
