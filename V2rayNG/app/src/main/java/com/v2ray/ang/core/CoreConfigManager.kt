...
object CoreConfigManager {
    ...
    private fun enableTrafficStats(config: JsonObject) {
        if (config.get("stats")?.isJsonObject != true) {
            config.add("stats", JsonObject())
        }

        val policy = config.get("policy")?.takeIf { it.isJsonObject }?.asJsonObject
            ?: JsonObject().also { config.add("policy", it) }
