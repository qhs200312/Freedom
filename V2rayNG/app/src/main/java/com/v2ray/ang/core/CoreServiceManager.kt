...
object CoreServiceManager {
    ...
    /**
     * Verifies that the running core can carry real outbound traffic. A listening SOCKS port
     * only proves that startup completed; the selected node can still be unreachable.
     * This method is blocking and must be called from a worker thread.
     */
    fun isCoreReachable(): Boolean {
        ...
        for (url in urls) {
            try {
                ...
                if (delay >= 0L) {
                    LogUtil.i(AppConfig.TAG, "StartCore-Manager: Core connectivity verified (${delay}ms)")
                    return true
                }
            } catch (e: Exception) {
                LogUtil.w(
                    AppConfig.TAG,
                    "StartCore-Manager: Connectivity check failed for $url: ${e.message}"
                )
            }
        }
        return false
    ...
    private fun launchCore(
        ...
        if (!isReload && notifyStartSuccess) {
        ...
        return try {
            ...
            for (attempt in 1..3) {
                ...
                try {
                    ...
                    if (!isCoreReachable()) {
                        throw IllegalStateException("Core connectivity check failed after reload")
                    }
                    lastError = null
                    break
                } catch (e: Exception) {
                    ...
                    LogUtil.w(
                        AppConfig.TAG,
                        "StartCore-Manager: Core reload attempt $attempt failed: ${e.message}"
                    )
                    if (isRunning()) {
            ...
            MessageHelper.sendMsg2UI(service, AppConfig.MSG_STATE_START_FAILURE, message)
            // Root modes keep redirecting traffic while the core is down. Stopping the service
            // removes those rules first, restoring direct connectivity instead of blackholing it.
