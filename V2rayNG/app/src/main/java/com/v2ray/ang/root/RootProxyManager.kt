...
object RootProxyManager {
    ...
    fun start(context: Context): Boolean {
        teardown(context)
        val script = if (SettingsManager.isTProxyMode()) {
            buildTProxySetup(context) ?: return false
        } else {
            buildTun2socksSetup(context) ?: return false
        }
        ...
        if (!isDataPathReachable(context)) {
            LogUtil.e(AppConfig.TAG, "RootProxyManager: data-path check failed, rolling back")
            teardown(context)
            return false
        }
        startDataPathWatchdog(context)
        return true
    }
    ...
    /**
     * Exercises the installed root route end to end. The command runs as uid 0, so it is
     * captured by both Root TUN and TProxy instead of taking the app-uid bypass.
     */
    fun isDataPathReachable(context: Context): Boolean {
        val urls = listOf(
            SettingsManager.getDelayTestUrl(),
            SettingsManager.getDelayTestUrl(true),
        ).distinct()
        val script = buildString {
            appendLine("command -v curl >/dev/null 2>&1 || exit 0")
            for (url in urls) {
                val safeUrl = url.replace("'", "'\\''")
                appendLine("curl -4 -fsS -o /dev/null --connect-timeout 6 --max-time 12 '$safeUrl' && exit 0")
            }
            appendLine("echo 'Root proxy data path is not reachable'")
            appendLine("exit 1")
        }
        val result = RootShell.runScript(context, "verify_data_path.sh", script)
        if (!result.success) {
            LogUtil.w(AppConfig.TAG, "RootProxyManager: data-path check output: ${result.output}")
        }
        return result.success
    }

    /** Root-side watchdog survives UI/process lifecycle glitches and always restores direct routing. */
    private fun startDataPathWatchdog(context: Context) {
        ...
        watchdogFile.writeText(buildString {
            appendLine("sleep 15")
            appendLine("command -v curl >/dev/null 2>&1 || exit 0")
            appendLine("failures=0")
            appendLine("while kill -0 $corePid 2>/dev/null; do")
    ...
    private fun buildTProxySetup(context: Context): String? {
        val bin = File(context.applicationInfo.nativeLibraryDir, AppConfig.ROOT_TPROXY_BIN)
        if (!bin.exists()) {
            LogUtil.e(AppConfig.TAG, "RootProxyManager: TProxy helper missing at ${bin.absolutePath}")
            return null
        }
        ...
        val selectedUids = if (perAppEnabled) {
            val packages = MmkvManager.decodeSettingsStringSet(AppConfig.PREF_PER_APP_PROXY_SET)
                ?.toList()
                .orEmpty()
            if (packages.isEmpty()) emptyList()
            else PackageUidResolver.packageNamesToUids(context, packages)
        } else {
            emptyList()
        }
        ...
        return buildString {
            ...
            append(waitForSocksListener(socksPort))
            appendLine("chmod 755 \"\$BIN\"")
            appendLine("cat > '$cfgFile' <<'TPROXYCFG'")
            append(buildTProxyConfig(socksUsername, socksPassword, logFile))
            appendLine("TPROXYCFG")
            appendLine("nohup \"\$BIN\" '$cfgFile' >/dev/null 2>&1 &")
            appendLine("TPROXY_PID=\$!")
            appendLine("echo \$TPROXY_PID > '$pidFile'")
            ...
            // Android's local TCP path needs the standard socket DIVERT rule. Without it,
            // SYN packets reach the transparent listener but the connection stalls in
            // SYN_RECV because subsequent packets are sent through TPROXY again.
            appendLine("iptables -t mangle -N $TPROXY_DIVERT_CHAIN 2>/dev/null || true")
            appendLine("iptables -t mangle -F $TPROXY_DIVERT_CHAIN")
            appendLine("iptables -t mangle -A $TPROXY_DIVERT_CHAIN -j MARK --set-xmark $MARK")
            appendLine("iptables -t mangle -A $TPROXY_DIVERT_CHAIN -j ACCEPT")
            appendLine("iptables -t mangle -D PREROUTING -p tcp -m socket -j $TPROXY_DIVERT_CHAIN 2>/dev/null || true")
            appendLine("iptables -t mangle -I PREROUTING 1 -p tcp -m socket -j $TPROXY_DIVERT_CHAIN")
            appendLine("iptables -t mangle -N $TPROXY_CHAIN 2>/dev/null || true")
            appendLine("iptables -t mangle -F $TPROXY_CHAIN")
            appendLine("iptables -t mangle -A $TPROXY_CHAIN -m mark --mark $MARK -p tcp -j TPROXY --on-port ${AppConfig.ROOT_TPROXY_PORT} --tproxy-mark $MARK/0xffffffff")
            appendLine("iptables -t mangle -A $TPROXY_CHAIN -m mark --mark $MARK -p udp -j TPROXY --on-port ${AppConfig.ROOT_TPROXY_PORT} --tproxy-mark $MARK/0xffffffff")
            appendLine("iptables -t mangle -D PREROUTING -j $TPROXY_CHAIN 2>/dev/null || true")
            appendLine("iptables -t mangle -A PREROUTING -j $TPROXY_CHAIN")
            // IPv6 blocking is best-effort on vendor kernels without a complete ip6tables.
            appendLine("set +e")
            append(buildV6Blackhole(appUid, perAppEnabled, bypassApps, selectedUids))
            appendLine("set -e")
            appendLine("kill -0 \$TPROXY_PID 2>/dev/null || { echo 'TProxy helper exited during setup'; exit 1; }")
            appendLine("ip rule list | grep -q 'fwmark.*lookup $TABLE' || { echo 'TProxy policy rule missing'; exit 1; }")
            appendLine("iptables -t mangle -C OUTPUT -j $CHAIN || { echo 'TProxy OUTPUT jump missing'; exit 1; }")
            appendLine("iptables -t mangle -C PREROUTING -j $TPROXY_CHAIN || { echo 'TProxy PREROUTING jump missing'; exit 1; }")
        }
    }

    private fun buildTProxyConfig(
        socksUsername: String?,
        socksPassword: String?,
        logFile: String,
    ): String = buildString {
        appendLine("main:")
        appendLine("  workers: 2")
        appendLine("socks5:")
        ...
        appendLine("tcp:")
        appendLine("  port: ${AppConfig.ROOT_TPROXY_PORT}")
    ...
    private fun buildTun2socksSetup(
        ...
        return buildString {
            appendLine("set -e")
            appendLine("BIN='${bin.absolutePath}'")
            append(waitForSocksListener(port))
            ...
            appendLine("set -e")
            appendLine("kill -0 \$T2S_PID 2>/dev/null || { echo 'tun2socks helper exited during setup'; cat '$logFile' 2>/dev/null; exit 1; }")
            appendLine("ip route show table $TABLE | grep -q 'dev $TUN' || { echo 'Root TUN route missing'; exit 1; }")
            appendLine("ip rule list | grep -q 'fwmark.*lookup $TABLE' || { echo 'Root TUN policy rule missing'; exit 1; }")
            if (captureDeviceTraffic) {
                appendLine("iptables -t mangle -C OUTPUT -j $CHAIN || { echo 'Root TUN OUTPUT jump missing'; exit 1; }")
            } else {
                appendLine("iptables -C FORWARD -j ${AppConfig.ROOT_FWD_CHAIN} || { echo 'LAN sharing FORWARD jump missing'; exit 1; }")
            }
        }
    }

    /** Wait until the in-process core can accept helper connections. */
    private fun waitForSocksListener(port: Int): String = buildString {
        appendLine("i=0")
        appendLine("while [ \$i -lt 30 ]; do")
        appendLine("  ss -lnt 2>/dev/null | grep -q ':$port ' && break")
        appendLine("  sleep 0.2")
        appendLine("  i=\$((i+1))")
        appendLine("done")
        appendLine("ss -lnt 2>/dev/null | grep -q ':$port ' || { echo 'Core SOCKS listener did not become ready on port $port'; exit 1; }")
    }
    ...
    private fun buildTeardown(context: Context): String {
        ...
        return buildString {
            ...
            for (cmd in listOf("iptables", "ip6tables")) {
                appendLine("del_all $cmd -t mangle -D OUTPUT -j $CHAIN")
                appendLine("$cmd -t mangle -F $CHAIN 2>/dev/null || true")
                appendLine("$cmd -t mangle -X $CHAIN 2>/dev/null || true")
            }
            appendLine("del_all iptables -t mangle -D PREROUTING -j $TPROXY_CHAIN")
            appendLine("iptables -t mangle -F $TPROXY_CHAIN 2>/dev/null || true")
            appendLine("iptables -t mangle -X $TPROXY_CHAIN 2>/dev/null || true")
            appendLine("del_all iptables -t mangle -D PREROUTING -p tcp -m socket -j $TPROXY_DIVERT_CHAIN")
            appendLine("iptables -t mangle -F $TPROXY_DIVERT_CHAIN 2>/dev/null || true")
            appendLine("iptables -t mangle -X $TPROXY_DIVERT_CHAIN 2>/dev/null || true")
            // IPv6 blackhole chain (only set up when v6 is disabled; harmless if absent)
            appendLine("del_all ip6tables -t filter -D OUTPUT -j ${AppConfig.ROOT_V6_CHAIN}")
            ...
            // routing rule + table
            appendLine("del_all ip rule del fwmark $MARK table $TABLE priority $PRIORITY")
            appendLine("del_all ip -6 rule del fwmark $MARK table $TABLE priority $PRIORITY")
            ...
            appendLine("del_all iptables -D FORWARD -j ${AppConfig.ROOT_FWD_CHAIN}")
            ...
            appendLine("del_all iptables -t mangle -D FORWARD -o $TUN -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --set-mss 1350")
            appendLine("del_all iptables -t nat -D PREROUTING -j ${AppConfig.ROOT_DNS_CHAIN}")
            ...
            appendLine("del_all ip6tables -D FORWARD -j ${AppConfig.ROOT_V6_FWD_CHAIN}")
            ...
            appendLine("del_all ip6tables -t mangle -D PREROUTING -j ${AppConfig.ROOT_V6_PRE_CHAIN}")
            ...
            for (pref in listOf(5000, 5010, 5020, 5025, 5026, 5027, 5030, 5040, 5050, 6000)) {
                appendLine("del_all ip rule del pref $pref")
            }
            ...
            // hev-socks5-tproxy handles SIGINT (not SIGTERM). Give it a short graceful
            // shutdown window, then force-stop only if the recorded process is still alive.
            appendLine("if [ -f '$tproxyPidFile' ]; then")
            appendLine("  TPROXY_PID=\$(cat '$tproxyPidFile')")
            appendLine("  kill -INT \$TPROXY_PID 2>/dev/null || true")
            appendLine("  i=0; while [ \$i -lt 10 ] && kill -0 \$TPROXY_PID 2>/dev/null; do sleep 0.1; i=\$((i+1)); done")
            appendLine("  kill -KILL \$TPROXY_PID 2>/dev/null || true")
            appendLine("fi")
